#!/bin/bash
# Phase D exp 3: Megatron PP=4 + fan-in rollout (SGLang fork, DP→TP switch),
# NO fused forward.
#
# Pair with baseline 1: (exp 3 − baseline 1) = fan-in gain on Megatron
# (isolated from fused forward).
#
# Fan-in: INITIAL=dp (4 independent DP workers for bulk) → triggers TP=4
# switch when active request count drops below threshold (min_idle=1).
# This is the "true" fan-in scenario we want to demonstrate.
#
# NOTE: INITIAL=dp path not yet pilot-validated with Megatron PP=4 training.
# Run smoke test first: TOTAL_STEPS=1 SEED=42 bash this_script.
set -o pipefail
set -x
rm -f log_rank_*.txt
...
GPU_MEMORY_UTILIZATION=0.5   # dual KV pools need headroom
