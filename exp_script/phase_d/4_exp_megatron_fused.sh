#!/bin/bash
# Phase D exp 2: Megatron PP=4 + fused forward, stock SGLang rollout DP=4.
#
# Pair with baseline 1: (exp 2 − baseline 1) = fused forward gain on Megatron
# (isolated from fan-in).
set -o pipefail
set -x
rm -f log_rank_*.txt
