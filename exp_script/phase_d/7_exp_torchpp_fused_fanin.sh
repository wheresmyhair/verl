#!/bin/bash
# Phase D exp 5: torch_pp PP=4 + fused forward + fan-in rollout (INITIAL=tp).
#
# Proves BOTH paper contributions work together on torch_pp backbone.
# Fan-in cycle: TP→DP (bulk) → TP (tail). INITIAL=tp avoids LogitsProcessor defect.
set -o pipefail
set -x
rm -f log_rank_*.txt
...
EXP_NAME=exp5_torchpp_fused_fanin
