#!/bin/bash
# Idea 2 baseline: torch_pp PP=4 with separate actor and log_prob passes
# (fused_forward=False). Pairs with idea2_fused.sh — the delta isolates
# the fused-forward contribution. Rollout side is vanilla SGLang DP=4.
set -o pipefail
set -x
rm -f log_rank_*.txt
