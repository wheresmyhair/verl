#!/bin/bash
# 16K baseline: rmpad + weight sync optimization, no progressive
set -x
rm -f log_rank_*.txt

PROFILING_DIR=/home/user/profiling_p3_baseline
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p3_16k_baseline.txt
