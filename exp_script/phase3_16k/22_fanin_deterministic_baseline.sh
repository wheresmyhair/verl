#!/bin/bash
# Fan-in functional test — deterministic baseline (DP=4, no fan-in).
# Qwen3-1.7B + DAPO-Math, temperature=0 (greedy), deterministic inference,
# n=1 (each prompt once → 16 requests, 4 per rank), 1 step.
# Purpose: establish per-prompt response lengths, show tail gap in trace.
set -x
rm -f log_rank_*.txt
...
export RLPIPE_SGFANIN_TRACE=/home/user/profiling_sgfanin_deterministic_baseline
export RLPIPE_FANIN_TOKEN_DUMP=/home/user/profiling_sgfanin_deterministic_baseline

PROFILING_DIR=/home/user/profiling_sgfanin_deterministic_baseline
rm -rf "$PROFILING_DIR"
...
python3 -m verl.trainer.main_ppo \
    ...
    2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p3_sgfanin_deterministic_baseline.txt
