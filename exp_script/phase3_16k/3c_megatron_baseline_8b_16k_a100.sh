#!/bin/bash
# Qwen3-8B / 16K response length Megatron PP=4 baseline on A100.
# Paper-relevant scale: long sequences with a larger model — the fused
# forward technique should benefit most here because compute_log_prob
# is a bigger fraction of step time at long seq lengths.
# Reduced batch size (16 from 64) to fit HBM at 16K.
set -x
rm -f log_rank_*.txt

export MEGATRON_CI_DISABLE_EXPANDABLE_SEGMENTS=1

PROFILING_DIR=/home/user/profiling_p3_megatron_baseline_8b_16k
...
YOUR_RUN_NAME=p3-8b-16k-megatron-baseline-a100
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_megatron_trainer' \
	...
	trainer.total_epochs=1 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p3_8b_16k_megatron_baseline.txt
