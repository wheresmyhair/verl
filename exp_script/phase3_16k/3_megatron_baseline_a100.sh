#!/bin/bash
# verl Megatron PP=4 baseline on A100 for 16K response length
# Used as the denominator for our fused forward Megatron port (Phase 1 MVP)
set -x
rm -f log_rank_*.txt
...
PROFILING_DIR=/home/user/profiling_p3_megatron_baseline
...
YOUR_RUN_NAME=p3-16k-megatron-baseline-a100
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_megatron_trainer' \
	...
	trainer.total_epochs=1 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p3_16k_megatron_baseline.txt
