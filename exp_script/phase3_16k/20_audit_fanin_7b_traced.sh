#!/bin/bash
# SGLang fan-in audit — Experiment 4/4.
# DeepSeek-R1-Distill-Qwen-7B + DAPO-Math-poc2, fork dynamic-TP + fan-in.
...
# rlpipe: step-level PPTracer, 4-track perfetto layout matching
# perfetto_example.json (-1 Trainer / 0 Phases / 1 GPU Ops / 5 HBM).
export RLPIPE_SGFANIN_TRACE=/home/user/profiling_sgfanin_audit_fanin_7b_traced
export RLPIPE_SGFANIN_FANIN_DEBUG=/home/user/profiling_sgfanin_audit_fanin_7b_traced/fanin_debug.log

export GLOO_SOCKET_TIMEOUT=7200
export TORCH_NCCL_DEFAULT_TIMEOUT_SECONDS=7200

# NOTE: expandable_segments incompatible with sglang fork torch_memory_saver.

PROFILING_DIR=/home/user/profiling_sgfanin_audit_fanin_7b_traced
...
# 0.50 chosen from 13_rlpipe_fanin_multistep.sh precedent — dual-pool +
# dual-graph peak HBM is ~4 GB higher than stock DP during switch windows.
GPU_MEMORY_UTILIZATION=0.50

MODEL_PATH="deepseek-ai/DeepSeek-R1-Distill-Qwen-7B"
YOUR_PROJECT_NAME=sgfanin-audit
YOUR_RUN_NAME=fanin-7b-traced
...
python3 -m verl.trainer.main_ppo \
    ...
    2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p3_sgfanin_audit_fanin_7b_traced.txt
