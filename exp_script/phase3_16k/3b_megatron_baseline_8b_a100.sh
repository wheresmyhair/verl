#!/bin/bash
# Qwen3-8B Megatron PP=4 baseline on A100 for 4K response length.
# Mirrors `4b_megatron_fused_forward_8b_a100.sh` config (same model, seq
# length, batch size, pp_size) but with fused forward DISABLED — used as
# the denominator for the 8B fused forward speedup claim.
set -x
rm -f log_rank_*.txt

export MEGATRON_CI_DISABLE_EXPANDABLE_SEGMENTS=1

PROFILING_DIR=/home/user/profiling_p3_megatron_baseline_8b
...
YOUR_RUN_NAME=p3-8b-megatron-baseline-a100
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_megatron_trainer' \
	...
	trainer.total_epochs=1 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p3_8b_megatron_baseline.txt
