#!/bin/bash
# #3 Het TP control: tp_groups=[2,1,1], fused forward, round_robin (no smart routing)
set -x
rm -f log_rank_*.txt

PROFILING_DIR=/home/user/profiling_p2_het_rr
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	actor_rollout_ref.rollout.routing_strategy=round_robin \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p2_het_rr.txt
