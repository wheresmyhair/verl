#!/bin/bash
# #4 Het TP + length routing: tp_groups=[2,1,1], fused forward, length (coefs 1.0/4.0)
set -x
rm -f log_rank_*.txt

PROFILING_DIR=/home/user/profiling_p2_het_length
...
YOUR_RUN_NAME=p2-het-tp-length
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p2_het_length.txt
