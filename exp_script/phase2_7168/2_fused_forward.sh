#!/bin/bash
# #2 Fused forward only: homo 4xTP=1, no routing, fused forward enabled
set -x
rm -f log_rank_*.txt

PROFILING_DIR=/home/user/profiling_p2_ff
...
YOUR_RUN_NAME=p2-fused-forward
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p2_ff.txt
