#!/bin/bash
# #5 Het TP + aggressive length routing: tp_groups=[2,1,1], fused forward, length (coefs 1.0/8.0)
set -x
rm -f log_rank_*.txt

PROFILING_DIR=/home/user/profiling_p2_het_resp8
...
YOUR_RUN_NAME=p2-het-tp-length-resp8
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	actor_rollout_ref.rollout.routing_response_coef=8.0 \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p2_het_resp8.txt
