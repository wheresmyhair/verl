#!/bin/bash
# #6 Het TP + pure response routing: tp_groups=[2,1,1], fused forward, response_length only
set -x
rm -f log_rank_*.txt

PROFILING_DIR=/home/user/profiling_p2_het_resp
...
YOUR_RUN_NAME=p2-het-tp-resp-only
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	actor_rollout_ref.rollout.routing_strategy=response_length \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p2_het_resp.txt
