#!/bin/bash
# verl Megatron PP=4 baseline: SGLang engine, same config as our torch PP baseline
set -x
rm -f log_rank_*.txt

# Blackwell RTX PRO 6000 workarounds (PCIe, no NVLink, Docker no SYS_PTRACE)
export NCCL_P2P_DISABLE=1
export NCCL_SHM_DISABLE=1
export CUDA_IPC_USE_PIDFD=0

PROFILING_DIR=/home/user/profiling_p2_verl_megatron
...
YOUR_RUN_NAME=p2-verl-megatron-pp4
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_megatron_trainer' \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p2_verl_megatron.txt
