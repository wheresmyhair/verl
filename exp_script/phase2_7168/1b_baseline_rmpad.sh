#!/bin/bash
# #1b Baseline + remove_padding: homo 4xTP=1, no routing, no fused forward, remove_padding=True
set -x
rm -f log_rank_*.txt

PROFILING_DIR=/home/user/profiling_p2_baseline_rmpad
...
YOUR_RUN_NAME=p2-baseline-rmpad
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p2_baseline_rmpad.txt
