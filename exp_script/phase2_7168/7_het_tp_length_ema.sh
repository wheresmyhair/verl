#!/bin/bash
# #7 Het TP + length routing with EMA estimator: tp_groups=[2,1,1], fused forward, 5 epochs
set -x
rm -f log_rank_*.txt

PROFILING_DIR=/home/user/profiling_p2_het_ema
...
YOUR_RUN_NAME=p2-het-tp-length-ema
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	actor_rollout_ref.rollout.routing_prompt_coef=1.0 \
	actor_rollout_ref.rollout.routing_response_coef=4.0 \
	actor_rollout_ref.rollout.routing_history_estimator=ema \
	actor_rollout_ref.rollout.routing_ema_alpha=0.3 \
	...
	trainer.total_epochs=5 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p2_het_ema.txt
