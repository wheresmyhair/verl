#!/bin/bash
# Progressive rollout: rmpad + weight sync + progressive_threshold=0.9
set -x
rm -f log_rank_*.txt

PROFILING_DIR=/home/user/profiling_p2_progressive
...
YOUR_RUN_NAME=p2-progressive-0.9
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	+actor_rollout_ref.rollout.progressive_threshold=0.9 \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_p2_progressive.txt
