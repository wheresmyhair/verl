#!/bin/bash
# Torch naive PP experiment with DP=4 vLLM rollout
# Same setup as run_vllm_megatron_dp4.sh but with torch_naive_pp strategy
set -x
rm -f log_rank_*.txt
...
python3 -m verl.trainer.main_ppo --config-path=./config --config-name='ppo_torch_pp_trainer' \
	...
	actor_rollout_ref.actor.num_micro_batches=4 \
	...
	trainer.total_epochs=3 2>&1 | sed 's/\x1b\[[0-9;]*m//g' | tee log_torch_pp.txt
