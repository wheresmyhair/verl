#!/bin/bash
# Idea 1 baseline: vanilla SGLang DP=4, no rlpipe fork, no fan-in.
# Pairs with idea1_fanin.sh — the delta isolates the fan-in contribution.
#
# Training side uses torch_pp + fused_forward=True so rollout is the
# only moving part between this script and idea1_fanin.sh.
set -o pipefail
set -x
rm -f log_rank_*.txt
